...
SELECT
    country,
	count(country) as number_of_movies_produced_in_2019
FROM
    movie
WHERE
...
min(total_votes) as min_t0tal_votes,
...
SELECT
	n.name as director_name,
		count(g.movie_id) as movie_count,
    row_number() over(order by count(r.movie_id)) as director_row_rank
from genre as g
	inner join ratings as r
    on g.movie_id = r.movie_id
...
SELECT 
    ...
    row_number() over(order by count(r.total_votes)) as prod_comp_rank
from movie as m
		inner join ratings as r
			on m.id = r.movie_id
...
FROM (
    SELECT
        ...
        COUNT(m.id) AS movie_count,
        SUM(avg_rating * r.total_votes) / SUM(r.total_votes) AS actress_avg_rating
    FROM
        movie AS m
    INNER JOIN
        role_mapping AS rm
...
SELECT 
    ...
    rank() over(order by m.worlwide_gross_income) as movie_rank
FROM
	movie AS m 
    INNER JOIN genre AS g 
    ON m.id= g.movie_id
